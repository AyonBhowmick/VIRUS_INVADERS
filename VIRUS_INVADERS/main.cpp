...
void drawText(float x, float y, const char* text, void* font = GLUT_BITMAP_HELVETICA_18) {
    ...
    float pixelX = (x + 1.0f) * 0.5f * width;
    float pixelY = (y + 1.0f) * 0.5f * height;
...
void renderFrontPage() {
    ...
    // University Header - Dark Blue for contrast
    glColor3f(0.0f, 0.2f, 0.4f); // Dark blue
    drawText(-0.45f, 0.85f, "AMERICAN INTERNATIONAL UNIVERSITY-BANGLADESH", GLUT_BITMAP_HELVETICA_18);

    glColor3f(0.0f, 0.3f, 0.6f); // Medium blue
    drawText(-0.15f, 0.75f, "Summer 2024-25", GLUT_BITMAP_HELVETICA_18);

    // Game Title - Bright color for emphasis
    glColor3f(0.8f, 0.0f, 0.0f); // Red
    drawText(-0.2f, 0.55f, "VIRUS INVADERS", GLUT_BITMAP_TIMES_ROMAN_24);

    // Course Information - Dark gray
    glColor3f(0.3f, 0.3f, 0.3f); // Dark gray
    drawText(-0.25f, 0.35f, "Course Teacher: NOBORANJAN DEY", GLUT_BITMAP_HELVETICA_18);

    // Group Members Header - Blue
    glColor3f(0.0f, 0.4f, 0.8f); // Blue
    drawText(-0.15f, 0.20f, "GROUP MEMBERS:", GLUT_BITMAP_HELVETICA_18);

    // Group Members List - Black for readability
    glColor3f(0.0f, 0.0f, 0.0f); // Black
    drawText(-0.8f, 0.05f, "GOLAM IFFAT RAHMAN FARAH", GLUT_BITMAP_HELVETICA_18);
    drawText(0.3f, 0.05f, "23-50230-1", GLUT_BITMAP_HELVETICA_18);
    drawText(0.6f, 0.05f, "BSc [CSE]", GLUT_BITMAP_HELVETICA_18);

    drawText(-0.8f, -0.05f, "AYON KUMAR BHOWMICK OVI", GLUT_BITMAP_HELVETICA_18);
    drawText(0.3f, -0.05f, "23-51050-1", GLUT_BITMAP_HELVETICA_18);
    drawText(0.6f, -0.05f, "BSc [CSE]", GLUT_BITMAP_HELVETICA_18);

    drawText(-0.8f, -0.15f, "KAZI WARISA TABASSUM", GLUT_BITMAP_HELVETICA_18);
    drawText(0.3f, -0.15f, "23-52623-2", GLUT_BITMAP_HELVETICA_18);
    drawText(0.6f, -0.15f, "BSc [CSE]", GLUT_BITMAP_HELVETICA_18);

    // Continue Instruction - Green
    glColor3f(0.0f, 0.6f, 0.0f); // Green
    drawText(-0.25f, -0.35f, "Press SPACE to Continue", GLUT_BITMAP_HELVETICA_18);
}
void renderStartScreen() {
    // Background
   // Background - Cosmic Purple Nebula
    glBegin(GL_QUADS);
    glColor3f(0.08f, 0.02f, 0.15f); // Deep purple
    glVertex2f(-1.0f, -1.0f);
    glColor3f(0.15f, 0.05f, 0.25f); // Medium purple
    glVertex2f(1.0f, -1.0f);
    glColor3f(0.08f, 0.02f, 0.15f); // Deep purple
    glVertex2f(1.0f, 1.0f);
    glColor3f(0.20f, 0.08f, 0.35f); // Light purple
    glVertex2f(-1.0f, 1.0f);
    glEnd();

    glColor3f(1.0f, 0.5f, 0.0f);
    drawText(-0.35f, 0.85f, "VIRUS INVADERS", GLUT_BITMAP_TIMES_ROMAN_24);

    // Game Description
    glColor3f(1.0f, 1.0f, 1.0f);
    drawText(-0.95f, 0.65f, "Welcome to Virus Invaders! Protect yourself from viruses");
    drawText(-0.95f, 0.55f, "by spraying them with disinfectant. Collect vaccines to");
    drawText(-0.95f, 0.45f, "restore your health and survive through 5 levels!");

    // Objectives Section
    glColor3f(0.0f, 1.0f, 1.0f);
    drawText(-0.95f, 0.25f, "OBJECTIVES:");

    glColor3f(1.0f, 1.0f, 1.0f);
    drawText(-0.95f, 0.15f, "- Destroy viruses with spray (SPACE key)");
    drawText(-0.95f, 0.05f, "- Avoid touching viruses (lose health)");
    drawText(-0.95f, -0.05f, "- Collect vaccines to restore health (max 3)");
    drawText(-0.95f, -0.15f, "- Complete 5 levels to win");
    drawText(-0.95f, -0.25f, "- Each level has different weather effects");

    // Controls Section
    glColor3f(0.0f, 1.0f, 1.0f);
    drawText(-0.95f, -0.40f, "CONTROLS:");

    glColor3f(1.0f, 1.0f, 1.0f);
    drawText(-0.95f, -0.50f, "Arrow Keys - Move your character");
    drawText(-0.95f, -0.60f, "SPACE - Spray disinfectant");
    drawText(-0.95f, -0.70f, "ESC - Quit game");

    // Level Information
    glColor3f(0.0f, 1.0f, 1.0f);
    drawText(0.1f, 0.25f, "LEVELS & WEATHER:");

    glColor3f(1.0f, 1.0f, 1.0f);
    drawText(0.1f, 0.15f, "Level 1: Sunny Day");
    drawText(0.1f, 0.05f, "Level 2: Night with Fireflies");
    drawText(0.1f, -0.05f, "Level 3: Winter with Snow");
    drawText(0.1f, -0.15f, "Level 4: Rainy Day");
    drawText(0.1f, -0.25f, "Level 5: Stormy Night");

    // Start Instruction
    glColor3f(0.0f, 1.0f, 0.0f);
    drawText(-0.25f, -0.85f, "Press SPACE to Start Game");
}
...
void spawnSprayBurst() {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        float speed = randRange(0.015f * scales, 0.03f * scales); // ← keep this exactly
...
void spawnVirus() {
    // Clear all existing viruses first
    for (int i = 0; i < 50; i++) {
        viruses[i].type = 0;
    }
    ...
    for (int i = 0; i < 50 && spawned < virusesToSpawn; i++) {
        Virus virus;
        int side = rand() % 4;
        ...
        virus.type = 1; // Always set to active
...
void renderGameOver() {
    ...
    glBegin(GL_QUADS);
    glColor4f(0.3f, 0.0f, 0.0f, 0.7f); // Semi-transparent red
    glVertex2f(-1.0f, -1.0f);
    glVertex2f(1.0f, -1.0f);
    glVertex2f(1.0f, 1.0f);
    glVertex2f(-1.0f, 1.0f);
    ...
    // Game Over message - PERFECTLY CENTERED
    glColor3f(1.0f, 0.0f, 0.0f);
    drawText(-0.12f, 0.3f, "GAME OVER", GLUT_BITMAP_TIMES_ROMAN_24);

    // Final Score - PERFECTLY CENTERED
    glColor3f(1.0f, 1.0f, 1.0f);
    char buf[64];
    snprintf(buf, sizeof(buf), "Final Score: %d", score);
    drawText(-0.10f, 0.15f, buf);

    // Instructions - PERFECTLY CENTERED
    drawText(-0.28f, -0.05f, "Press ENTER to view High Scores");
    drawText(-0.18f, -0.15f, "Press ESC to quit");
}
void renderHighScores() {
    ...
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    glBegin(GL_QUADS);
    glColor4f(0.0f, 0.0f, 0.3f, 0.7f); // Semi-transparent blue
    glVertex2f(-1.0f, -1.0f);
    glVertex2f(1.0f, -1.0f);
    glVertex2f(1.0f, 1.0f);
    glVertex2f(-1.0f, 1.0f);
    ...
    glColor3f(0.0f, 1.0f, 0.0f);
    drawText(-0.12f, 0.8f, "HIGH SCORES", GLUT_BITMAP_TIMES_ROMAN_24);

    glColor3f(1.0f, 1.0f, 1.0f);
    char buf[64];
    for (int i = 0; i < MAX_HIGH_SCORES; i++) {
        snprintf(buf, sizeof(buf), "%d. %d", i + 1, highScores[i]);
        drawText(-0.06f, 0.6f - (i * 0.1f), buf);
    }

    snprintf(buf, sizeof(buf), "Your Score: %d", score);
    drawText(-0.06f, 0.0f, buf);

    if (scoreAdded) {
        glColor3f(1.0f, 1.0f, 0.0f);
        drawText(-0.08f, -0.1f, "New High Score!");
    }

    glColor3f(1.0f, 1.0f, 1.0f);
    drawText(-0.08f, -0.2f, "Press R to Restart");
    drawText(-0.06f, -0.3f, "Press ESC to quit");
}
void drawMessageBoundary(float x, float y, const char* text, void* font = GLUT_BITMAP_TIMES_ROMAN_24) {
    // Calculate the text width and height (approximation)
    float textWidth = 0.0f;
    float textHeight = 20.0f;

    // Estimate the text width
    for (const char* c = text; *c; c++) {
        textWidth += glutBitmapWidth(font, *c); // Get width of each character
    }

    // Estimate the text height based on the font size
    textHeight = 18.0f; // Approximate height for GLUT_BITMAP_TIMES_ROMAN_24

    // Add some padding to the text box
    float padding = 0.05f;

    // Draw rectangle (boundary) around the message
    glBegin(GL_QUADS);
    glVertex2f(x - (textWidth / 2.0f) - padding, y - (textHeight / 2.0f) - padding);  // Bottom left
    glVertex2f(x + (textWidth / 2.0f) + padding, y - (textHeight / 2.0f) - padding);  // Bottom right
    glVertex2f(x + (textWidth / 2.0f) + padding, y + (textHeight / 2.0f) + padding);  // Top right
    glVertex2f(x - (textWidth / 2.0f) - padding, y + (textHeight / 2.0f) + padding);  // Top left
    glEnd();
}
...
glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    glBegin(GL_QUADS);
    glColor4f(0.0f, 0.3f, 0.0f, 0.7f); // Semi-transparent green
    glVertex2f(-1.0f, -1.0f);
    glVertex2f(1.0f, -1.0f);
    glVertex2f(1.0f, 1.0f);
    glVertex2f(-1.0f, 1.0f);
    ...
    // Level Complete message box - PROPERLY SCALED FOR 800x800
    glColor3f(0.0f, 0.0f, 0.0f);
    glBegin(GL_QUADS);
    glVertex2f(-0.5f, 0.25f);   // Wider box for 800x800
    ...
    drawText(-0.25f, 0.33f, "LEVEL COMPLETE!", GLUT_BITMAP_TIMES_ROMAN_24);

    // Score display - PERFECTLY CENTERED
    glColor3f(1.0f, 1.0f, 1.0f);
    char buf[64];
    snprintf(buf, sizeof(buf), "Score: %d", score);
    drawText(-0.05f, 0.15f, buf);

    // If Level 5, show special completion message
    if (level == 5) {
        glColor3f(0.0f, 1.0f, 1.0f); // Cyan color
        drawText(-0.35f, 0.0f, "CONGRATULATIONS! YOU BEAT THE GAME!");

        glColor3f(1.0f, 1.0f, 0.0f); // Yellow color
        drawText(-0.30f, -0.1f, "THANK YOU FOR PLAYING VIRUS INVADERS!");

        // Instructions - PERFECTLY CENTERED
        glColor3f(1.0f, 1.0f, 1.0f);
        drawText(-0.12f, -0.25f, "Press R to Play Again");
        drawText(-0.08f, -0.35f, "Press ESC to Quit");
    } else {
        // For other levels, show countdown - PERFECTLY CENTERED
        int secondsLeft = static_cast<int>(levelTransitionTimer) + 1;
        snprintf(buf, sizeof(buf), "Next Level Starting in %d...", secondsLeft);
        drawText(-0.18f, -0.05f, buf);
    }
}
void drawHighlightedText(float x, float y, const char* text, void* font = GLUT_BITMAP_TIMES_ROMAN_24) {
    // Set the highlight background color (e.g., light yellow)
    glColor3f(1.0f, 1.0f, 0.0f); // Light yellow for highlight
    ...
    // Draw rectangle (boundary) around the message (highlighted background)
    glBegin(GL_QUADS);
    glVertex2f(x - (textWidth / 2.0f) - padding, y - (textHeight / 2.0f) - padding);  // Bottom left
    glVertex2f(x + (textWidth / 2.0f) + padding, y - (textHeight / 2.0f) - padding);  // Bottom right
    glVertex2f(x + (textWidth / 2.0f) + padding, y + (textHeight / 2.0f) + padding);  // Top right
    ...
    // Draw the actual message on top of the highlighted background
    glColor3f(0.0f, 0.0f, 0.0f); // Black color for text
    drawText(x, y, text, font);
}


// Function to draw a rectangle (boundary) around a message
...
void updateGame() {
    ...
    if (level == 1) {
        ...
        currentWeather = NIGHT_WITH_RAIN;
    }
    ...
    if (currentWeather == RAIN || currentWeather == NIGHT_WITH_RAIN) {
        updateRaindrops();
    }
    if (currentWeather == NIGHT || currentWeather == NIGHT_WITH_RAIN) {
        updateFireflies();
    }
    ...
    for (int i = 0; i < 100; i++) {
        if (sprays[i].life > 0.0f) {
            ...
            // Remove spray if out of bounds or life expired
            if (sprays[i].life <= 0.0f ||
                ...
                sprays[i].pos.y < -1.1f || sprays[i].pos.y > 1.1f)
            {
                sprays[i].life = 0.0f;
                sprays[i].r = 0.0f;
                sprays[i].pos = {0.0f, 0.0f};
                sprays[i].vel = {0.0f, 0.0f};
            }
        }
    }

    // Spawn new sprays if sprayActive
    if (sprayActive) {
        spawnSprayBurst();
        score++;
    }
    ...
    for (int i = 0; i < 50; i++) {
        ...
        // Keep viruses within bounds
        viruses[i].pos.x = clampf(viruses[i].pos.x, -1.0f, 1.0f);
        viruses[i].pos.y = clampf(viruses[i].pos.y, -1.0f, 1.0f);
    }

    // Check for collision between sprays and viruses
    for (int i = 0; i < 50; i++) {
        if (viruses[i].type == 0) continue;
        ...
        if (hit) {
            viruses[i].type = 0; // Mark virus as inactive
        }
    }
    ...
    int target = level * 5;
    if (virusesKilled >= target && !isTransitioning) {
        if (level < 5) {
            isTransitioning = true;
            levelTransitionTimer = LEVEL_TRANSITION_TIME;
            gameState = LEVEL_COMPLETE;
        } else {
            // Do not set GAME_OVER, stay in LEVEL_COMPLETE for Level 5
            isTransitioning = false;
            gameState = LEVEL_COMPLETE;

            // Disable virus spawning after Level 5
            for (int i = 0; i < 50; i++) {
                viruses[i].type = 0; // Deactivate all viruses
            }
        }
    }

    // Spawn more viruses if needed
    int activeViruses = 0;
    for (int i = 0; i < 50; i++) {
        if (viruses[i].type != 0) {
            activeViruses++;
...
void keyboardDown(unsigned char key, int, int) {
    ...
    else if (gameState == LEVEL_COMPLETE && level == 5) {
        if (key == 'r' || key == 'R') {
            // Restart game from level 5 completion
            gameState = PLAYING;
            player.health = 3;
            score = 0;
            virusesKilled = 0;
    ...
    // Remove the LEVEL_COMPLETE space handler since we're using timer now
...
int main(int argc, char** argv) {
    ...
    initParkObjects();
   // spawnVirus();
   // Initialize all sprays to safe default values
for (int i = 0; i < 100; i++) {
    sprays[i].life = 0.0f;
    sprays[i].r = 0.0f;
    sprays[i].pos = {0.0f, 0.0f};
